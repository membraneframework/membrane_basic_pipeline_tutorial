  ...
  describe "Source" do
    ...
    test "reads the input file correctly" do
      with_mock File, [read!: fn _ -> "First Line\nSecond Line" end] do
        {{:ok, _}, state} = Source.handle_stopped_to_prepared(nil, %{location: @exemplary_location, content: nil})
        assert state.content == @exemplary_content
      end
    end

    test "supplies the buffers" do
      {{:ok, actions}, state} = Source.handle_demand(:output, 1, :buffers, nil, %{location: @exemplary_location, content: @exemplary_content})
    ...
    test "redemands if more then one buffer is demanded" do
      {{:ok, actions}, state} = Source.handle_demand(:output, 2, :buffers, nil, %{location: @exemplary_location, content: @exemplary_content})
      assert length(state.content) == length(@exemplary_content)-1
      assert actions == [buffer: {:output, %Buffer{payload: Enum.at(@exemplary_content, 0)}}, redemand: :output]
    end

    test "sends end of stream once there are no buffers" do
      {{:ok, actions}, state} = Source.handle_demand(:output, 1, :buffers, nil, %{location: @exemplary_location, content: []})
