defmodule OrderingBufferTest do
  ...
  test "Ordering buffer should order the incoming packets" do
    {{:ok, actions}, state} = OrderingBuffer.handle_process(:input, %Buffer{payload: "[seq:2]How are"}, nil, @initial_state)
    assert actions == [redemand: :output]
    {{:ok, actions}, state} = OrderingBuffer.handle_process(:input, %Buffer{payload: "[seq:3] you?"}, nil, state)
    assert actions == [redemand: :output]
    {{:ok, actions}, state} = OrderingBuffer.handle_process(:input, %Buffer{payload: "[seq:7]Something else"}, nil, state)
    assert actions == [redemand: :output]
    {{:ok, actions}, state} = OrderingBuffer.handle_process(:input, %Buffer{payload: "[seq:1]Hello! "}, nil, state)
