defmodule DepayloaderTest do
  ...
  test "Depayloader should assemble the packets and form a frame (with membrane's testing framework based on generator)" do
    initial_state = ["[frameid:1s][timestamp:1]Hello! ", "[frameid:1][timestamp:1]How are", "[frameid:1e][timestamp:1] you?"]
    ...
    options = %Pipeline.Options{
      elements: [
        source: %Source{output: {initial_state, generator}, caps: %Basic.Formats.Packet{type: :custom_packets}},
        depayloader: %Depayloader{packets_per_frame: 5},
        sink: Sink
      ]
    }
