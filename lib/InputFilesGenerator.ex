defmodule InputFilesGenerator do
  ...
  defp prepare_sequence(content) do
    content = for {{words_list, timestamp}, frame_id} <- Enum.with_index(content, 1) do
    ...
    content = content |> Enum.with_index(1) |> Enum.map(fn {sequence, sequence_id} -> "[seq:#{sequence_id}]#{sequence}" end)
